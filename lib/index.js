...
function parser() {
  ...
  function special_unshift(_node) {
    ...
    for(var i = 0, len = this.length; i < len; ++i)
      pad += ' |' 
    console.log(pad + '\\' + _node.type + ': ' + _node.id + ':\t\t' + (_node.token ? JSON.stringify(_node.token.data) : '<UNK>'))
    }
  ...
  function parse_decllist() {
    ...
    if(token.type === 'operator') {
      ...
      if(token.data === ',') {
        // multi-decl!
        if(!state[1].allow_comma) return state.shift()

        return tokens.shift()
      } else if(token.data === '=') {
        if(!state[1].allow_assign) return unexpected('`=` is not allowed here.')
  ...
  function decl(allow_assign, allow_comma) {
    ...
    parse_forloop = 
        ...
        , function() {
            ...
            if(token.type === 'ident') {
              ...
              if(lookup.parent.type === 'struct') {
                return state.unshift(decl()), Advance
              }
    ...
    parse_function_args =
      stative(
        function() {
          ...
          state.unshift(decl(NO_ASSIGN_ALLOWED, NO_COMMA_ALLOWED))
          return Advance
        }
      , function() {
